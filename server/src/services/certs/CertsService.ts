...
export class CertsService implements CertsServiceInterface {
    ...
    async getCerts(edrpou: string): Promise<Cert[]> {
        ...
        try {
            ...
            const response: AxiosResponse<CertsApiResponse> = await axios.post(`${this.apiUrl}/cert/info.php`, {
                getInfoByEdrpou: 1,
                edrpou: edrpou
            }, {
                headers: {
            ...
            if (data.status === 'ok') {
                this.logger.info(`Successfully fetched ${data.data.length} certificates for EDRPOU: ${edrpou}`);
                return data.data;
            } else {
